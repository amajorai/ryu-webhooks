...
- **`ui/` — companion (companion-only app, no backend crate).** A sandboxed
  ...
  whether a signing secret is set, and last-delivery timestamps. Workflow endpoints
  can read, save, or server-generate their signing secret through an explicit
  protected call. Composio's project subscription owns its secret and Core syncs it
  automatically; the normal registry response never contains secret material.
...
producers to a node. Workflow secret values are blurred by default in the editor
and reveal on hover or focus. **Generate** asks Core for a fresh random value;
**Save** accepts a custom value of at least 16 characters. Composio instead shows
its provider-managed state because Ryu reconciles the URL and secret during trigger
setup.

## Star History

<a href="https://github.com/amajorai/ryu-webhooks/stargazers">
  <picture>
